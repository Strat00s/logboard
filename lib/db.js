...
CREATE TABLE IF NOT EXISTS channels (
  ...
  expires_at TEXT,
  default_md  INTEGER,          -- render default for the UI: NULL unset, 0 off, 1 on
  default_col INTEGER,
...
class Store {
  constructor(file, { pendingDays = 10 } = {}) {
    ...
    this.db.exec(SCHEMA);
    // Render defaults arrived after the first schema: add the columns to
    // existing databases. NULL means "not set — inherit".
    for (const table of ['channels', 'threads']) {
      const cols = this.db.prepare(`PRAGMA table_info(${table})`).all().map((c) => c.name);
      for (const col of ['default_md', 'default_col']) {
        if (!cols.includes(col)) this.db.exec(`ALTER TABLE ${table} ADD COLUMN ${col} INTEGER`);
      }
    }
  ...
  // Per-target render defaults the UI seeds from on page load: NULL = unset
  // (inherit channel, then the browser setting), 0 = off, 1 = on.
  setRenderDefaults(kind, id, { md, col }) {
    if (kind !== 'channels' && kind !== 'threads') throw new HttpError(500, 'bad render target');
    const must = kind === 'channels' ? this.mustGetChannel(id) : this.mustGetThread(id);
    const norm = (v) => {
      if (v === null || v === 'null') return null;
      if (v === true || v === 1 || v === '1' || v === 'on') return 1;
      if (v === false || v === 0 || v === '0' || v === 'off') return 0;
      throw new HttpError(400, 'render defaults must be true, false or null');
    };
    const sets = [];
    const args = [];
    if (md !== undefined) { sets.push('default_md = ?'); args.push(norm(md)); }
    if (col !== undefined) { sets.push('default_col = ?'); args.push(norm(col)); }
    if (sets.length) {
      this.db.prepare(`UPDATE ${kind} SET ${sets.join(', ')} WHERE id = ?`).run(...args, Number(must.id));
      return kind === 'channels' ? this.mustGetChannel(must.id) : this.mustGetThread(must.id);
    }
    return must;
  }
  ...
  // Global counters for the sidebar line and the settings dialog. `bytes` is
  // the stored message payload; the db file itself is store.dbBytes().
  counts(id = null) {
    const g = this.db
      .prepare(
        `SELECT (SELECT COUNT(*) FROM messages) AS messages,
                ...
                (SELECT COALESCE(SUM(bytes), 0) FROM messages) AS bytes,
  ...
  // Anything that changes what the server *does*, no matter who is watching,
  // belongs in the meta table — the browser's localStorage is for looks.
  ...
  // Delete every message older than `days` days from now. Timestamps are all
  // normalised UTC ISO strings, so plain string comparison orders correctly.
  deleteOlderThan(days, now = nowIso()) {
    const cutoff = new Date(Date.parse(now) - days * 86_400_000).toISOString();
    const r = this.db.prepare('DELETE FROM messages WHERE ts < ?').run(cutoff);
    return { messages: r.changes, cutoff };
  }

  // What the board costs on disk: main file plus the WAL/shm sidecars.
  dbBytes() {
    if (this.file === ':memory:') return 0;
    let n = 0;
    for (const suffix of ['', '-wal', '-shm']) {
      try { n += fs.statSync(this.file + suffix).size; } catch { /* not created yet */ }
    }
    return n;
  }
